...
const ColorSliderComponent = (props: PropsColorSlider) => {
  return (
    <div>
      <input type="range" min="0" max="255" value={props.value}
      onChange={event => props.onValueUpdated(+event.target.value)}></input>
      {props.value}
    </div>
  )
}
  ...
  <>
    {Object.keys(props.color).map((field: keyof Color) => (
      <ColorSliderComponent 
        ...
        onValueUpdated={updateColor(props, field)}></ColorSliderComponent>
    ))}
  </>
)